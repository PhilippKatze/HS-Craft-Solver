...
solve skill attributes mindurability lvl allIngredients = do
    ...
    _ <- mapConcurrently (solvePart skyline (V.fromList attributes) mindurability allIngrd) multiChunks
    --sequence_ $ parMap rseq (solvePart skyline (V.fromList attributes) mindurability allIngrd) multiChunks
        ...
        (\p -> V.forM_ allIng 
            (\a1 -> V.forM_ allIng 
                (\a2 -> V.forM_ allIng 
                    (\a3 -> V.forM_ allIng 
                        (\a4 -> V.forM_ allIng 
                            (\a5 -> testForSkyline attributes skyline $ createReceipe mindurability attributes (V.fromList[p,a1,a2,a3,a4,a5])))))))
--TODO cleanup? xD
...
testForSkyline :: V.Vector String -> MVar (V.Vector RecipeResult) -> Maybe RecipeResult -> IO ()
